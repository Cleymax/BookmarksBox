        ...
        render: function() {
            ...
            this.$list = new  PhpDebugBar.Widgets.ListWidget({ itemRenderer: function(li, tpl) {
                ...
                if (tpl.render_time_str) {
                    $('<span title="Temps de rendu" />').addClass(csscls('render-time')).text(tpl.render_time_str).appendTo(li);
                }
                if (tpl.memory_str) {
                    $('<span title="Usage de la mémoire" />').addClass(csscls('memory')).text(tpl.memory_str).appendTo(li);
                }
                if (typeof(tpl.param_count) != 'undefined') {
                    $('<span title="Nombre de paramètres" />').addClass(csscls('param-count')).text(tpl.param_count).appendTo(li);
                }
