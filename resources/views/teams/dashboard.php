...
if (isset($id) && Auth::check() && TeamHelper::canManage($id)) {
    ?>
...
if (Auth::check() && TeamHelper::canEdit($id)) {
    ?>
...
<div class="grid-250">
    ...
    foreach ($folders as $folder) {
        ...
        <a href="<?= get_query_url('/teams/' . $id . '/folder/' . $folder->id) ?>">
            <div class="folder"
                 style="color: <?php echo strtolower($folder->color) == '#ffffff' ? 'var(--dark)' : $folder->color ?>"
                 folder-id="<?= $folder->id ?>">
